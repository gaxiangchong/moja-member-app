...
export function cartSubtotalCents(lines: { unitPriceCents: number; qty: number }[]): number {
...
export function computeDiscountCents(
  ...
  if (voucher) {
    if (voucher.discountType === 'percent') {
      return Math.min(Math.floor((subtotalCents * voucher.value) / 100), subtotalCents);
    }
