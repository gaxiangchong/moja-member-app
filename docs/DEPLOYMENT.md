...
Read `reports/performance/baseline.md` for the median summary and `baseline.json` for raw samples. Every sample uses an independent cold-cache browser context. A real control is clicked for INP; unavailable Event Timing data is warned and never treated as a pass. A warning means the page exceeded LCP 2.5 s, CLS 0.1, or INP 200 ms under the simulated 390×844, slow-4G, 4× CPU profile. These thresholds are initially non-blocking; compare the JSON `schemaVersion`, page IDs, metric keys, and sample counts between runs when checking report stability.
