...
import type { UpdatePaymentsDemoModeDto } from './dto/update-payments-demo-mode.dto';
...
export class AdminService {
  ...
  /** Effective PAYMENTS_DEMO_MODE value read the same way PaymentsService reads it, ignoring any override. */
  private paymentsDemoModeEnvDefault(): boolean {
    const v = this.config
  ...
  getPaymentsDemoMode() {
    ...
    const envDefault = this.paymentsDemoModeEnvDefault();
    return {
      override,
      envDefault,
  ...
  async setPaymentsDemoMode(
    dto: UpdatePaymentsDemoModeDto,
    auth: AdminAuthState,
  ) {
    const before = this.paymentsSettings.getDemoModeOverride();
    const next = this.paymentsSettings.setSettings({
      demoModeOverride: dto.enabled ?? null,
    });
    await this.audit.log({
      ...auditActorBase(auth),
      action: 'payments.demo_mode_toggled',
      entityType: 'payments_settings',
      beforeValue: { demoModeOverride: before } as object,
      afterValue: { demoModeOverride: next.demoModeOverride } as object,
    });
    return this.getPaymentsDemoMode();
  }
